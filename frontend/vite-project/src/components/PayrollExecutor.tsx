  ...
  return (
    <div className="card">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-lg font-semibold">Execute Payroll</h2>
      </div>

      {payments.length > 0 && (
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="bg-yellow-900/20 border border-yellow-800/30 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-yellow-400">{routes.yellowPayments.length}</div>
            <div className="text-xs text-yellow-500 mt-1">Yellow Network</div>
            <div className="text-[10px] text-gray-500">Same-chain batch</div>
          </div>
          <div className="bg-purple-900/20 border border-purple-800/30 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-purple-400">{routes.lifiPayments.length}</div>
            <div className="text-xs text-purple-500 mt-1">LI.FI Routes</div>
          ...
          <div className="bg-blue-900/20 border border-blue-800/30 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-blue-400">{routes.suiPayments.length}</div>
            <div className="text-xs text-blue-500 mt-1">Sui PTB</div>
            <div className="text-[10px] text-gray-500">Atomic batch</div>
          </div>
        </div>
      )}

      {savings.savingsPercent > 0 && (
        <div className="bg-accent-900/20 border border-accent-800/30 rounded-lg p-3 mb-4 flex items-center justify-between">
          <div>
            <div className="text-sm text-accent-400 font-medium">Gas Savings</div>
            <div className="text-xs text-gray-500 mt-0.5">
              {savings.withoutPayflow} txns <ArrowRight size={10} className="inline" /> {savings.withPayflow} txns
            </div>
          </div>
          <div className="text-3xl font-bold text-accent-400">{savings.savingsPercent}%</div>
        </div>
      )}
        ...
        <div className="mb-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-gray-400">
              {phase === 'yellow' && 'Processing Yellow Network session...'}
              {phase === 'lifi' && 'Routing cross-chain via LI.FI...'}
              {phase === 'sui' && 'Executing Sui PTB batch...'}
            </span>
          </div>
      ...
      <button
        ...
        className="w-full btn-primary flex items-center justify-center gap-2 py-3 text-base font-semibold disabled:opacity-50"
      >
        {isExecuting ? (
          <>
          ...
          <>
            <Play size={20} />
            Payroll Complete - Run Again
          </>
        ) : (
          <>
            <Play size={20} />
            Execute Payroll ({pendingCount} recipients)
          </>
        )}
      </button>

      {!isEvmConnected && (
        <p className="text-xs text-gray-500 text-center mt-2">Connect EVM wallet to execute</p>
      )}
      ...
      {phase === 'done' && (
        <button
          ...
          className="w-full mt-2 flex items-center justify-center gap-2 py-2 text-sm text-gray-400 hover:text-gray-300 border border-gray-700 rounded-lg hover:border-gray-600 transition-colors"
        >
          <RotateCcw size={16} />
          Reset Recipients
        </button>
      ...
      {phase === 'done' && (
        <div className="mt-4 flex items-center justify-center gap-2 text-accent-400">
          ...
          <span className="text-sm">All payments processed successfully!</span>
        </div>
      )}
    </div>
  )
