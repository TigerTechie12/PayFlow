import { useMemo } from 'react'
import { ArrowRightLeft, TrendingDown, Layers } from 'lucide-react'
import { WalletConnect } from './WalletConnect'
import { RecipientTable } from './RecipientTable'
import { PayrollExecutor } from './PayrollExecutor'
import { TransactionStatus } from './TransactionStatus'
import { usePayFlowStore } from '../lib/store'
import { categorizePayments, recipientsToPayments, calculateTotalAmount } from '../lib/payflowEngine'

export function Dashboard() {
  const { recipients, payerChain } = usePayFlowStore()

  const payments = useMemo(() => recipientsToPayments(recipients), [recipients])
  const routes = useMemo(() => categorizePayments(payments, payerChain), [payments, payerChain])

  const totalAmount = useMemo(() => calculateTotalAmount(payments), [payments])

  return (
    <div className="min-h-screen bg-gray-950">
      <header className="border-b border-gray-800 bg-gray-950/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between py-3 sm:py-0 sm:h-16 gap-3 sm:gap-0">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-gradient-to-br from-primary-500 to-secondary-500 flex items-center justify-center flex-shrink-0">
                <ArrowRightLeft size={16} className="text-white" />
              </div>
              <div>
                <h1 className="text-base sm:text-lg font-bold bg-gradient-to-r from-primary-400 to-secondary-400 bg-clip-text text-transparent">
                  PayFlow
                </h1>
                <p className="text-[9px] sm:text-[10px] text-gray-500 -mt-0.5 hidden sm:block">Universal Cross-Chain Payroll</p>
              </div>
            </div>
            <WalletConnect />
          </div>
        </div>
      </header>

      <main className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6">
        {recipients.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-4 sm:mb-6">
            <StatCard
              icon={<Layers size={14} />}
              label="Recipients"
              value={recipients.length.toString()}
              color="text-primary-400"
            />
            <StatCard
              icon={<ArrowRightLeft size={14} />}
              label="Amount"
              value={parseFloat(totalAmount) > 0 ? totalAmount : '0'}
              color="text-secondary-400"
            />
            <StatCard
              icon={<TrendingDown size={14} />}
              label="Chains"
              value={new Set(recipients.map((r) => r.chain)).size.toString()}
              color="text-accent-400"
            />
            <StatCard
              icon={<Layers size={14} />}
              label="Protocols"
              value={[
                routes.yellowPayments.length > 0 ? 'Yellow' : '',
                routes.lifiPayments.length > 0 ? 'LI.FI' : '',
                routes.suiPayments.length > 0 ? 'Sui' : '',
              ].filter(Boolean).length.toString()}
              color="text-yellow-400"
            />
          </div>
        )}

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          <div className="lg:col-span-2 space-y-4 sm:space-y-6">
            <RecipientTable />
          </div>

          <div className="space-y-4 sm:space-y-6">
            <PayrollExecutor />
            <TransactionStatus />
          </div>
        </div>

        <div className="mt-6 sm:mt-8 card p-4 sm:p-6">
          <h3 className="text-xs sm:text-sm font-semibold text-gray-400 mb-3 sm:mb-4 uppercase tracking-wider">
            How PayFlow Works
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
            <ProtocolCard
              name="Yellow Network"
              description="Same-chain payments batched via state channels."
              color="from-yellow-500/20 to-yellow-600/5"
              borderColor="border-yellow-800/30"
              tag="Gasless"
            />
            <ProtocolCard
              name="LI.FI Protocol"
              description="Cross-chain payments via bridges and DEXs."
              color="from-purple-500/20 to-purple-600/5"
              borderColor="border-purple-800/30"
              tag="Cross-Chain"
            />
            <ProtocolCard
              name="Sui PTB"
              description="Atomic batch execution on Sui blockchain."
              color="from-blue-500/20 to-blue-600/5"
              borderColor="border-blue-800/30"
              tag="Atomic"
            />
          </div>
        </div>
      </main>

      <footer className="border-t border-gray-800 mt-8 sm:mt-12 py-4 sm:py-6">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 text-center">
          <p className="text-[10px] sm:text-xs text-gray-600">
            PayFlow - Yellow Network + LI.FI + Sui
          </p>
        </div>
      </footer>
    </div>
  )
}

function StatCard({
  icon,
  label,
  value,
  color,
}: {
  icon: React.ReactNode
  label: string
  value: string
  color: string
}) {
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-lg sm:rounded-xl p-2.5 sm:p-4">
      <div className={`flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs text-gray-500 mb-0.5 sm:mb-1`}>
        <span className={color}>{icon}</span>
        {label}
      </div>
      <div className="text-lg sm:text-2xl font-bold text-white">{value}</div>
    </div>
  )
}

function ProtocolCard({
  name,
  description,
  color,
  borderColor,
  tag,
}: {
  name: string
  description: string
  color: string
  borderColor: string
  tag: string
}) {
  return (
    <div className={`bg-gradient-to-b ${color} border ${borderColor} rounded-lg p-3 sm:p-4`}>
      <div className="flex items-center justify-between mb-1.5 sm:mb-2">
        <h4 className="text-xs sm:text-sm font-semibold text-white">{name}</h4>
        <span className="text-[8px] sm:text-[10px] px-1.5 sm:px-2 py-0.5 bg-gray-900/50 rounded-full text-gray-400">
          {tag}
        </span>
      </div>
      <p className="text-[10px] sm:text-xs text-gray-400 leading-relaxed">{description}</p>
    </div>
  )
}
